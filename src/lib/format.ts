...
/**
 * Ввод суммы: цифры + один разделитель + до 2 знаков после него. Разряды
 * группируются ПРОБЕЛАМИ, поэтому разделитель всегда однозначно читается как
 * десятичный: «1500.5» → «1 500.5».
 *
 * Запятая принимается наравне с точкой (на части раскладок она под рукой), но
 * показывается всегда точка — её и набирают на буквенной клавиатуре iOS, и
 * в таком же виде суммы отображаются во всём остальном приложении.
 */
export function grpAmount(raw: string): string {
  const s = (raw || '').replace(/[^\d.,]/g, '') // пробелы-разряды и мусор отпадают
  const sep = s.search(/[.,]/)
  // Ведущие нули убираем: «0012» — это 12. Один ноль оставляем, иначе
  // «0.5» превратилось бы в «.5», а поле — в пустое при вводе одного нуля.
  const int = (sep >= 0 ? s.slice(0, sep) : s)
    .replace(/\D/g, '')
    .replace(/^0+(?=\d)/, '')
    .slice(0, 13)
